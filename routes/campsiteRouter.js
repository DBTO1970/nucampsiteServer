...
campsiteRouter
  ...
  .put(authenticate.verifyUser, (req, res, next) => {
    // return campsite by id of request (url)

    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        //test to see if user id is same as id of author of comment
        console.log(req.user._id);
        console.log(campsite.comments.id(req.params.commentId).author);
        if (campsite && campsite.comments.id(req.params.commentId)) {
          //check if current user is the same as author of comment
            if (
            campsite.comments
            ...
            err.status = 403; //error for not the correct author
            return next(err);
          }
        } else if (!campsite) {
          err = new Error(`Campsite ${req.params.campsiteId} not found`);
